    ...
    const AddToLayoutParams& params)
{
    ...
    QnLayoutItemData data = QnLayoutItemData::createFromResource(resource);
...
void ActionHandler::at_openInNewTabAction_triggered()
{
    ...
    for (const auto& layout: layouts)
    {
        ...
        if (!wbLayout)
        {
            ...
            if (!wbLayout->isSearchLayout())
            {
                ...
                if (layout->isFile())
                {
                    state = QnStreamSynchronizationState(
                        /*isLive*/ false, /*position*/ 0, /*speed*/ 1.0);
                }
                ...
                for (auto item: wbLayout->items())
                {
                    ...
                    {
                        item->setData(Qn::ItemTimeRole, state.timeUs == DATETIME_NOW
                                ? DATETIME_NOW
                                : state.timeUs / 1000);
    ...
    if (const auto widgets = parameters.widgets(); widgets.isEmpty())
    {
...
{
    qnClientModule->clientStateHandler()->createNewWindow(/*logIn*/ context()->user(), {});
}
    ...
    if (server) //< Null server is for webpages only, cameras cannot be moved out of the server.
    {
        ...
        if (!nonMovableCameras.empty())
        {
            ...
            if (!groupName.isEmpty() && nonMovableCameras.size() < camerasToMove.size())
            {
                ...
                if (moveAnyways)
                {
                    const auto toRemove = nonMovableCameras.toSet();
...
void ActionHandler::at_thumbnailsSearchAction_triggered()
{
    ...
    for (int i = 0; i < itemCount; i++) {
        ...
        QnLayoutItemData item = QnLayoutItemData::createFromResource(resource);
...
void ActionHandler::checkIfStatisticsReportAllowed()
{
    ...
    const bool atLeastOneServerHasInternetAccess = resourcePool()->serverWithInternetAccess();
