...
struct BookmarkItem
{
    std::optional<QnCameraBookmark> bookmark;
...
bool bookmarkItemLess(const BookmarkItemPtr &left, const BookmarkItemPtr &right)
{
    ...
    if (left->bookmark && right->bookmark)
        return left->bookmark->guid < right->bookmark->guid;
...
void QnBookmarkMergeHelper::removeBookmark(const QnCameraBookmark &bookmark)
{
    ...
    if (it == zeroLevel.items.end() || (*it)->bookmark->guid != bookmark.guid)
        return;
    ...
    while (parent)
    {
        ...
        if (parent->bookmark)
        {
            ...
            if (parentIt == zeroLevel.items.end() || (*parentIt)->bookmark->guid != bookmark.guid)
            {
...
void QnBookmarkMergeHelperPrivate::mergeBookmarkItems(int detailLevel)
{
    ...
    for (BookmarkItemPtr &item: prevLevel.items)
    {
        ...
        if (!currentItem)
        {
            if (item->bookmark)
            {
                currentItem = BookmarkItemPtr(new BookmarkItem(*item->bookmark));
            }
            else
            {
