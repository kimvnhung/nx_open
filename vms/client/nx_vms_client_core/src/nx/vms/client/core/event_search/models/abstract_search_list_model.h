...
public:
    ...
    Q_INVOKABLE virtual bool canFetchData() const;
    ...
    EventSearch::FetchDirection fetchDirection() const;
    Q_INVOKABLE void setFetchDirection(nx::vms::client::core::EventSearch::FetchDirection value);
    ...
    Q_INVOKABLE virtual bool fetchInProgress() const;
    virtual bool cancelFetch();

signals:
    void fetchCommitStarted(nx::vms::client::core::EventSearch::FetchDirection direction, QPrivateSignal);
    void fetchFinished(
        nx::vms::client::core::EventSearch::FetchResult result,
        nx::vms::client::core::EventSearch::FetchDirection direction,
        QPrivateSignal);
...
protected:
    ...
    Q_INVOKABLE virtual void requestFetch() = 0;
    ...
    // Helper to truncate data sorted in descending timestamp order.
    // @returns True if some data was truncated, false otherwise.
    template<class DataContainer>
    bool truncateDataToMaximumCount(DataContainer& data,
        GetTimestampFunction<typename DataContainer::value_type> getTimestamp,
        ItemCleanupFunction<typename DataContainer::value_type> itemCleanup = nullptr);
...
private:
    UserWatcher* const m_userWatcher;
    ...
    EventSearch::FetchDirection m_fetchDirection =
        EventSearch::FetchDirection::earlier; //< Direction for next fetch.
...
template<class DataContainer>
bool AbstractSearchListModel::truncateDataToMaximumCount(
    DataContainer& data, //< Must be sorted in descending order by timestamp!
    GetTimestampFunction<typename DataContainer::value_type> getTimestamp,
    ItemCleanupFunction<typename DataContainer::value_type> itemCleanup)
{
...
//        timeWindow.truncateFront(getTimestamp(data.back()).count());
    ...
    setFetchedTimeWindow(data.empty()
        ? QnTimePeriod()
        : QnTimePeriod::fromInterval(getTimestamp(data.back()), getTimestamp(data.front())));
    return true;
}

} // namespace nx::vms::client::core

//Q_DECLARE_METATYPE(nx::vms::client::core::AbstractSearchListModel::FetchResult)
