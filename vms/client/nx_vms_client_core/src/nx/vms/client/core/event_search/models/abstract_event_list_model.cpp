...
AbstractEventListModel::AbstractEventListModel(
    ...
    :
    base_type(parent),
    m_serverTimeWatcher(serverTimeWatcher)
{
    NX_CRITICAL(m_serverTimeWatcher, "Server time watcher must be specified.");
}
...
QVariant AbstractEventListModel::data(const QModelIndex& index, int role) const
{
    ...
    switch (role)
    {
        case TimestampTextRole:
            return timestampText(index.data(TimestampRole).value<microseconds>());
...
{
    return timestampText(std::chrono::milliseconds(timestampMs));
}
...
QString AbstractEventListModel::timestampText(microseconds timestamp) const
{
    ...
    const QDateTime dateTime = m_serverTimeWatcher->displayTime(timestampMs);
