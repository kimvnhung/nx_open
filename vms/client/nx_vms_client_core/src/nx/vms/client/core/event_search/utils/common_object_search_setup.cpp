...
QnCommonModule* CommonObjectSearchSetup::commonModule() const
{
    return d->commonModule();
}

void CommonObjectSearchSetup::setCommonModule(QnCommonModule* value)
{
    d->setCommonModule(value);
}
...
bool CommonObjectSearchSetup::mixedDevices() const
{
    ...
    return resourcePool && resourcePool->containsIoModules();
}
    ...
    m_dayChangeTimer->callOnTimeout(this,
        [this]()
        {
            ...
            const auto timeWatcher =
                commonModule()->instance<nx::vms::client::core::ServerTimeWatcher>(); //< Todo: move server time watcher to the ocmmon everywhere
...
QnCommonModule* CommonObjectSearchSetup::Private::commonModule() const
{
...
void CommonObjectSearchSetup::Private::setCommonModule(QnCommonModule* value)
{
    if (m_commonModule == value)
        return;

    m_commonModule = value;
...
void CommonObjectSearchSetup::Private::setTimeSelection(EventSearch::TimeSelection value)
{
    ...
    if (m_commonModule && m_timeSelection != EventSearch::TimeSelection::selection)
...
{
    if (!NX_ASSERT(m_commonModule && m_model))
        return false;
    ...
    const auto cameras = nx::utils::toQSet(
        m_commonModule->resourcePool()->getResourcesByIds<QnVirtualCameraResource>(chosenIds));

    if (cameras.empty())
        return false; //< Cancel, if user didn't select any camera.

    if (m_cameraSelection == EventSearch::CameraSelection::custom && cameras == m_model->cameras())
