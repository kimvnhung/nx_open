...
int QnCameraBookmarksManagerPrivate::getTailBookmarksAsync(
    const QnTailBookmarkSearchFilter& filter,
    ...
    TailBookmarksCallbackType callback)
{
    ...
    QnGetBookmarksRequestData requestData;
    requestData.format = Qn::SerializationFormat::UbjsonFormat;
    requestData.filter.orderBy.column = nx::vms::api::BookmarkSortField::startTime;
    requestData.filter.text = filter.text;
    requestData.filter.cameras = filter.cameras;
    requestData.filter.startTimeMs = filter.referencePointMs < filter.startPointMs
        ? QnTimePeriod::kMinTimeValue //< Look for the earliest possible bookmarks.
        : filter.startPointMs; //< Start search from the the earliest available bookmark timestamp.
    requestData.filter.endTimeMs = filter.referencePointMs < filter.startPointMs
        ? filter.startPointMs //< End search at the the oldest available bookmark timestamp.
        : QnTimePeriod::kMaxTimeValue; //< Look for the earliest possible bookmarks.
    requestData.filter.orderBy.order = filter.startPointMs < filter.referencePointMs
        ? Qt::AscendingOrder
        : Qt::DescendingOrder;
    requestData.filter.startTimeMs =
        std::max(requestData.filter.startTimeMs, filter.minStartTimeMs);

    if (filter.bodyLimit != QnCameraBookmarkSearchFilter::kNoLimit
        && filter.tailLimit != QnCameraBookmarkSearchFilter::kNoLimit)
    {
        requestData.filter.limit = filter.bodyLimit + filter.tailLimit;
    }

    const auto direction = filter.startPointMs < filter.referencePointMs
        ? EventSearch::FetchDirection::later
        : EventSearch::FetchDirection::earlier;

    const auto handler = makeUbjsonResponseWrapper(BookmarksCallbackType(
        [this, source, callback, filter, direction, triesCount]
            (bool success, rest::Handle requestId, QnCameraBookmarkList bookmarks)
        {
            ...
            if (!success)
            {
                callback(/*success*/ false, requestId, /*fetched*/ {});
                return;
            }

            auto fetched = reorderAroundReferencePoint<BookmarkAccessor>(
                source, bookmarks, direction);
            const auto checkResult = checkFetchedData(
                fetched, filter.bodyLimit, filter.tailLimit, direction);

            if (checkResult.success)
            {
                truncateFetchedData(fetched, direction, filter.bodyLimit, filter.tailLimit);
                callback(/*success*/ true, requestId, fetched);
            }
            else if (checkResult.nextStartPointMs.count() && triesCount > 0)
            {
                QnTailBookmarkSearchFilter updatedFilter = filter;
                updatedFilter.startPointMs = checkResult.nextStartPointMs;
                getTailBookmarksAsync(updatedFilter, source, triesCount - 1, callback);
            }
            else
            {
                callback(/*success*/ false, requestId, /*fetched*/ {});
