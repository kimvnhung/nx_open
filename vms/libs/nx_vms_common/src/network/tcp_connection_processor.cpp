...
QnAuthSession QnTCPConnectionProcessor::authSession(const Qn::UserAccessData& accessRights) const
{
    ...
    nx::String existSession = nx::network::http::getHeaderValue(d->request.headers, Qn::AUTH_SESSION_HEADER_NAME);
    if (!existSession.isEmpty()) {
        result.fromString(existSession);
        return result;
    }
    ...
    if (result.id.isNull())
    {
        nx::String nonce = d->request.getCookieValue("auth");
        if (!nonce.isEmpty())
        {
    ...
    result.userHost = QString::fromStdString(nx::network::http::getHeaderValue(d->request.headers, Qn::USER_HOST_HEADER_NAME));
    if (result.userHost.isEmpty())
        result.userHost = d->socket->getForeignAddress().address.toString().c_str();
    result.userAgent = query.queryItemValue(QLatin1String(Qn::USER_AGENT_HEADER_NAME));
    if (result.userAgent.isEmpty())
        result.userAgent = QString::fromStdString(nx::network::http::getHeaderValue(d->request.headers, Qn::USER_AGENT_HEADER_NAME));
