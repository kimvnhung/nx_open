...
protected:
    void runStreamingTest(bool doServerDelay, bool doClientDelay)
    {
        ...
        if (doServerDelay)
        {
            ASSERT_TRUE(clientSocket.setSendTimeout(duration_cast<milliseconds>(kClientDelay).count()));
        }
...
private:
    ...
    void streamingServerMain(bool doServerDelay)
    {
        ...
        SystemError::ErrorCode closureCode = SystemError::noError;
        for (;;)
        {
            ...
            {
                closureCode = SystemError::getLastOSErrorCode();
                if (closureCode != SystemError::timedOut &&
                    closureCode != SystemError::again &&
                    closureCode != SystemError::interrupted &&
                    closureCode != SystemError::wouldBlock)
                {
        ...
        ASSERT_EQ((std::size_t)wholeData.size(), kBufferSize * sizeof(int) * kIterations)
            << SystemError::toString(closureCode);
